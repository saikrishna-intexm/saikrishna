...
$this->db->update('leads'); // gives UPDATE mytable SET field = field+1 WHERE id = 2
    ...
    public function createThreeLead($data, $pone, $ptwo, $pthree){
        ...
        if(isset($offer->offer_id)){
            ...
            if($offer->pincode_check == 'Y') {
                    ...
                    $this->db->select('*');
                    $this->db->where('mobile', "91". $lead->mobile);
                    $exist = $this->db->get('exist_kotak')->row();
                    
                    if(isset($exist->mobile) AND $exist->mobile > 0){
                        $ex = 1;
                    } else {
                        $ex = 0;
                    }
                    
                    if(isset($pincode->pincode_id) AND $pincode->pincode_id > 0 AND $ex == 0){
                       $offer_id = $offer->offer_id;
                    } else {
                        $this->db->select('*');
                        $this->db->where('min_income <=', $lead->income);
                        ...
                        if(isset($offer->offer_id) AND $lead->employee_type == 'SELF-EMPLOYEE'){
    ...
    public function getBackURL(){
        $this->db->select('*');
        $this->db->where('type','back_url');
        $result = $this->db->get('settings')->row()->description;
        return $result;
    }
