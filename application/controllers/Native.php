...
class Native extends CI_Controller {
	...
	function __construct() {
		...
		$this->load->model('native_model');
	}
	
	public function index()
	{
        $this->load->view('native/form');
	}
    
	public function form()
	{
		$this->load->view('native/form_'.$this->uri->segment(3));
	}
	...
	public function loan_application(){
		...
		if(isset($mobile) && $mobile != ''){
			...
			$result = $this->native_model->createLead($data, $pone, $ptwo);
	...
	public function page(){
		...
		$data['offer'] = $this->native_model->getOfferById($offer_id);
		$data['lead'] = $this->native_model->getLeadById($lead_id);
		$this->load->view('native/'. $data['offer']->offer_page .'', $data);
	}
    
    
	public function offer_page($page){
		$this->load->view('native/'. $page);
	}
