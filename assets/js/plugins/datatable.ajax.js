...
class RowsAjax {
  constructor() {
    ...
    this._createFileUploadInstance();
    this._addListeners();
    this._extend();
  ...
  _createInstance() {
    ...
    this._datatable = jQuery('#datatableRowsAjax').DataTable({
      ...
      info: false,
      ajax: 'http://redirect.com:8014/admin/get_campaigns',
      ...
      pageLength: 10,
      columns: [{data: 'campaign_id'}, {data: 'campaign_name'}, {data: 'priority_url_one'}, {data: 'priority_url_two'}, {data: 'Tag'}, {data: 'Check'}],
      language: {
        paginate: {
    ...
    // Creating datatable instance. Table data is provided by json/products.json file and loaded via ajax
  _createFileUploadInstance() {
    const _this = this;
    this._datatable = jQuery('#datatableRowsAjax2').DataTable({
      scrollX: true,
      buttons: ['copy', 'excel', 'csv', 'print'],
      info: false,
      ajax: 'http://redirect.com:8014/admin/get_file_uploads_list',
      order: [], // Clearing default order
      sDom: '<"row"<"col-sm-12"<"table-container"t>r>><"row"<"col-12"p>>', // Hiding all other dom elements except table and pagination
      pageLength: 10,
      columns: [{data: 'file_upload_id'}, {data: 'file_name'}, {data: 'campaign_id'}, {data: 'created_at'}, {data: 'Tag'}, {data: 'Check'}],
      language: {
        paginate: {
          previous: '<i class="cs-chevron-left"></i>',
          next: '<i class="cs-chevron-right"></i>',
      ...
      columnDefs: [
        ...
        // Adding Tag content as a span with a badge class
        {
          targets: 4,
          render: function (data, type, row, meta) {
            return '<span class="badge bg-outline-primary" onclick="myDownload(' + row['file_upload_id'] + ')" style="cursor: pointer;">Download</span>';
          },
        },
        // Adding checkbox for Check column
        {
